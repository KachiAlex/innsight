...
publicPortalRouter.get('/:tenantSlug/availability', async (req, res) => {
  ...
  const { startDate, endDate, categoryId, minOccupancy, includeOutOfOrder } = parsed.data;
  ...
  const occupiedRoomIds = new Set(overlappingReservations.map((reservation) => reservation.roomId));

  const availableRooms = rooms
    .filter((room) => !!room.id && !occupiedRoomIds.has(room.id))
    .map((room) => ({
      id: room.id,
      ...
      ratePlan: room.ratePlan
        ...
        : null,
      customRate: decimalToNumber(room.customRate),
    }));
...
publicPortalRouter.post('/:tenantSlug/checkout/confirm', async (req, res) => {
  ...
  const reservation = await prisma.reservation.create({
    data: {
      ...
      depositAmount: depositAmountValue !== null ? new Prisma.Decimal(depositAmountValue) : null,
      depositStatus,
      depositRequired,
